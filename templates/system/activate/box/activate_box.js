Vue.component('activate-box', {
	...
	methods: {
        check: function(event) {
            ...
            var self = this;
            new daoclient.DaoBox().check({ip:self.$root.user.local_ip, email:self.email}, this.$root.user.token)
                .done(function(data) {
                    console.log(data);
                    var ip_central = data[0].fields.ip;
                    console.log(ip_central);
                    var win = window.open("http://" + ip_central + ":8080/inovefila/#/box", '_blank');
                    console.log(win);
                    win.focus();
        ...
        activate: function(event) {
            ...
            new daoclient.DaoBox().activate({ip:self.ip, email:self.email, chave:self.chave}, this.$root.user.token)
                .done(function(data) {
                    ...
                    self.$root.$router.push({name: "activate-box"});
                })
                .fail(function(data) {
                    console.log(data.responseText);
                    self.$root.mostrar_msg(data.responseText);
